...
pub async fn start_monitor() -> Result<()> {
    ...
    loop {
        ...
        if *MONITOR_PAUSED.lock().unwrap() { continue; }
        let app_name = app_launcher::get_frontmost_app_name();

        // Text capture
        if let Ok(content) = clipboard.get_text() {
            if content != last_text && !content.trim().is_empty() {
                if should_skip_app(&app_name) { last_text = content; continue; }
                if let Err(e) = save_clipboard_entry(&content, &app_name).await { eprintln!("Failed to save clipboard entry: {}", e); }
                last_text = content;
            }
        }
        // Image capture
        if let Ok(image) = clipboard.get_image() {
            ...
            if changed {
                if should_skip_app(&app_name) { last_image_hash = Some(hash); continue; }
                if let Err(e) = save_clipboard_image(&image, &app_name).await { eprintln!("Failed to save clipboard image: {}", e); }
...
pub async fn delete_entry(id: i64) -> Result<()> {
    ...
    if is_pinned == 1 {
        return Err(anyhow::anyhow!("Cannot delete pinned item. Unpin it first."));
    }
