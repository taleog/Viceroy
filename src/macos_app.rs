...
unsafe fn register_search_delegate_class() {
    macos_search::register_search_delegate_class();
}
    ...
    if objc::runtime::Class::get("MKMenuActions").is_none() {
        ...
        extern "C" fn show_clipboard_history(_this: &Object, _cmd: Sel, _sender: id) {
            unsafe {
                ...
                macos_search::abort_current_search();
                show_clipboard_history_view();
            }
        }
...
unsafe fn bring_window_to_front_only(window: id) {
    ...
    // Snappy scale pop without exceeding 1.0 to avoid clipping.
    let content: id = msg_send![window, contentView];
    if content != nil {
        unsafe { scale_bounce_show(content, 0.18, 0.94) };
    }
}
...
unsafe fn reset_search_state() {
    ...
    if let Some(search_field) = macos_search::find_search_field() {
    ...
    extern "C" fn window_did_become_key(_this: &Object, _cmd: Sel, _notification: id) {
        unsafe {
            ...
            if count > 0 {
                ...
                } else if let Some(search_field) = macos_search::find_search_field() {
                    let _: () = msg_send![window, makeFirstResponder: search_field];
                }
            }
        }
